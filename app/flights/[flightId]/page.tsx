...
export default async function BookingPage({
  ...
  await connectDB();
  const flight = await Flight.findById(flightId).lean() as any;
  ...
  const flightForForm = JSON.parse(JSON.stringify(flight));
  ...
  return (
    <main style={{ background: "#f8f7f4", minHeight: "100vh", fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "80px 40px" }}>
        ...
        <a href="/flights/search" style={{ fontSize: "12px", letterSpacing: "0.15em", textTransform: "uppercase", color: "#888", textDecoration: "none", display: "inline-block", marginBottom: "48px" }}>← Back to Search</a>
        <p style={{ fontSize: "11px", letterSpacing: "0.4em", textTransform: "uppercase", color: "#888", marginBottom: "12px" }}>{flight.flightNumber} · {flight.aircraft}</p>
        <h1 style={{ fontSize: "clamp(40px, 6vw, 72px)", fontWeight: 900, letterSpacing: "-0.03em", margin: "0 0 60px", lineHeight: 1 }}>BOOK FLIGHT</h1>

        <div style={{ background: "white", borderRadius: "4px", padding: "48px", marginBottom: "4px" }}>
          <p style={{ fontSize: "11px", letterSpacing: "0.3em", textTransform: "uppercase", color: "#888", marginBottom: "24px" }}>{formatDate(departure)}</p>
        ...
        <div style={{ background: "white", borderRadius: "4px", padding: "48px" }}>
          ...
          <BookingForm flight={flightForForm} />
        </div>
